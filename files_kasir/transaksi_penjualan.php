                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div class="input-group input-group">
                                        ...
                                        <input type="text" name="harga_jual" id="harga_jual" required class="form-control harga_jual" onkeypress='return angka(event)'>
										<input type="hidden" name="harga_beli" class="harga_beli">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <div class="input-group input-group">
                                        <input type="text" name="quantity" id="quantity" required class="form-control quantity" onkeypress='return angka(event)'>
                        ...
                        <div class="col-md-7">
                            <form class="form-horizontal">
                                ...
                                <div class="form-group" style="margin-top:-10px;">
                                    <label class="col-sm-6 control-label">Total Diskon</label>
                                    <div class="col-sm-6">
                                        <input type="text"  class="form-control input-sm total_diskon input_ket" value="0" style="text-align:right;" disabled> 
                                    </div>
                                </div> 
...
$(document).ready(function () {
    ...
    $('#nama_barang').on('select2:select', function(e) {
        ...
        $.ajax({
			...
			success     : function(data) {
                //alert(data['satuan']+data['harga_jual']);
    ...
    $('#list_penjualan').bootstrapTable({
		columns:[	
				...
				}, 
                {
				...
				}, 
				 {
                    ...
                    formatter: function (value, row) {
					    return 	[  	'<input type="text" id="'+row.id+'"  value="'+row.diskon+'"  class="form-control input-sm tbl_diskon" style="width:80px; text-align:right; margin-top:-4px;">' 
								];
					}
				...
				}, 
				{
                    ...
                    width:100,
                    formatter: function (value, row) {
					    return 	[  	'<input type="text" id="'+row.id+'" value="'+row.quantity+'" class="form-control input-sm tbl_quantity" style="width:100px; text-align:center; margin-top:-4px;">' 
								];
					}
...
//==================== UPDATE quantity PADA TABLE ====================================//

    $(document).on('keydown','.tbl_quantity',function(e){
        if ( (e.which == 13)|(e.which == 9) ) {
            quantity = $(this).val();
            id = $(this).attr('id');
            update_quantity_table(id,quantity);
            
        } 
    });


    $(document).on('blur','.tbl_quantity',function(e){
            quantity = $(this).val();
            id = $(this).attr('id');
            update_quantity_table(id,quantity);
    });

    function update_quantity_table(id,quantity){
        $.ajax({
                url         :"./kelas/item_post.php",
                type        : "POST",
                data        :{op:"update_quantity_tmp",quantity:quantity,id:id},
                cache       :false,
                success:function(data){
                    load_data_penjualan();
                },
...
//=======================================================================//
//==================== UPDATE diskon  PADA TABLE ====================================//
$(document).on('keydown','.tbl_diskon',function(e){
        if ( (e.which == 13)|(e.which == 9)) {
            diskon = $(this).val();
            id = $(this).attr('id');
            update_diskon_table(id,diskon);
        } 
    });

    $(document).on('blur','.tbl_diskon',function(e){
        diskon = $(this).val();
            id = $(this).attr('id');
            update_diskon_table(id,diskon);
    });

    function update_diskon_table(id,diskon){
        $.ajax({
                url         :"./kelas/item_post.php",
                type        : "POST",
                data        :{op:"update_diskon_tmp",diskon:diskon,id:id},
                cache       :false,
                success:function(data){
                    load_data_penjualan();
                },
    ...
    function load_data_penjualan(){
		...
		$.ajax({
			...
			success     : function(data) {
                    ...
                    $('.total_diskon').val(data['detail_penjualan_list'][0]['total_diskon']);
...
//================================== hitung ==========================================//
    function hitung_total_bayar(){
        ...
        total_diskon       = parseInt($(".total_diskon").val().replace(/[^,\d]/g, '').toString());


        total_bayar          = Intl.NumberFormat().format(total_belanja-total_diskon);   
